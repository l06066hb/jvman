    ...
    def _load_all_translations(self):
        ...
        try:
            for lang in ["en_US", "zh_CN"]:
                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Successfully loaded translations for {lang}"
                                )
                            except json.JSONDecodeError as je:
                                # 提供更详细的 JSON 解析错误信息
                                error_line = content.splitlines()[je.lineno - 1]
                                logger.error(
                                    f"JSON parse error in {lang}.json at line {je.lineno}, column {je.colno}:"
                                )
                        ...
                        logger.error(
                            f"Failed to read translation file {file_path}: {str(e)}"
                        )
                        translations = {}
                else:
                ...
                if not translations:
                    logger.warning(
                        f"No translations loaded for {lang}, using empty dictionary"
                    )
                    translations = {}
