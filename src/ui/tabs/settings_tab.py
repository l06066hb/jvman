...
from PyQt6.QtCore import pyqtSignal, Qt, QTimer, QThread, QMetaObject, Q_ARG
...
class SettingsTab(QWidget):
    ...
    def _to_absolute_path(self, relative_path):
        ...
        try:
            ...
            logger.debug(
                f"Converting relative path '{relative_path}' to absolute path '{abs_path}'"
            )
    ...
    def apply_env_settings(self):
        ...
        try:
            ...
            # 保存设置到配置文件（使用相对路径）
            self.config.set(
                "jdk_store_path", self._to_relative_path(self.store_path_edit.text())
            )
            ...
            if success:
                ...
                QMessageBox.warning(
                    self,
                    _("settings.error.title"),
                    _("settings.messages.env_update_partial_failed").format(
    ...
    def update_env_preview(self):
        ...
        if current_path != _("settings.env.not_set"):
            ...
            for path in current_path.split(path_sep):
                ...
                if current_java_home != _("settings.env.not_set") and path.startswith(
                    current_java_home
                ):
                    path = path.replace(
                        current_java_home, f"{var_prefix}JAVA_HOME{var_suffix}"
                ...
                # 高亮显示 Java 相关路径
                if (
                    any(java_key in path.lower() for java_key in ["java", "jdk", "jre"])
