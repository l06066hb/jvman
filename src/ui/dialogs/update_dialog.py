...
class UpdateNotificationDialog(QDialog):
    ...
    def init_ui(self):
        ...
        if self.update_info.get('changelog_url'):
            ...
            changelog_link.clicked.connect(
                lambda: QDesktopServices.openUrl(QUrl(self.update_info['changelog_url']))
            )
            button_layout.addWidget(changelog_link, alignment=Qt.AlignmentFlag.AlignLeft)
        ...
        if self.update_info.get('alternative_package'):
            ...
            alt_link = QPushButton(
                _("update.dialog.download_alternative").format(
                    type=alt_package['type']
                )
            )
    ...
    def update_speed(self):
        ...
        if time_diff > 0:
            bytes_per_second = (current_progress - self.last_progress) * 1024 * 1024 / 100 / time_diff
            ...
            if bytes_per_second > 0:
                ...
                remaining_bytes = remaining_percentage * self.update_info['file_size'] / 100
                remaining_seconds = remaining_bytes / bytes_per_second
                if remaining_seconds < 60:
                    time_text = f"{int(remaining_seconds)} {_('update.dialog.seconds')}"
                else:
                    time_text = f"{int(remaining_seconds/60)} {_('update.dialog.minutes')}"
